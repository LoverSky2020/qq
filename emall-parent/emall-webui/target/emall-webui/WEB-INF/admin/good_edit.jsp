...
<div class="container">
	...
	<br><br>
	<form class="form-horizontal" action="admin/goods/edit" method="post" enctype="multipart/form-data" onsubmit="return check()">
			...
			<div class="col-sm-6">
				<select class="form-control" id="select_topic" name="typeId">
					<c:forEach var="type" items="${types}">
						<c:if test="${type.id==good.typeId}"><option selected="selected" value="${type.id}">${type.name}</option></c:if>
						<c:if test="${type.id!=good.typeId}"><option value="${type.id}">${type.name}</option></c:if>
					</c:forEach>
				</select>
			</div>
		</div>
